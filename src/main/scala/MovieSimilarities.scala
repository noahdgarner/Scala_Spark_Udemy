...
object MovieSimilarities {
  ...
  //since were passing this movieIds, we keep the one with a smaller
  //id, the smaller is not important the important part is the
  //filtering out of doubles and either direction
  def filterDuplicates(userRatings:UserRatingPair):Boolean = {
    val movieRating1 = userRatings._2._1
    val movieRating2 = userRatings._2._2

    //._1 gives movieId, the first val in the tuple
    val movie1 = movieRating1._1
    val movie2 = movieRating2._1
  ...
  def main(args: Array[String]) {
    ...
    //multiple users doing multiple ratings on multiple movies
    val ratings = data
      .map(l => l.split("\t"))
    ...
    // We now have (movie1, movie2) => (rating1, rating2) in each line
    // Now collect all ratings for each movie pair and compute similarity
    val moviePairRatings = moviePairs
      .groupByKey()

    // We now have (movie1, movie2) = > (rating1, rating2), (rating1, rating2) ...
    // Can now compute similarities. (Only passes values to function)
