...
object AverageFriendsByAge {
  ...
  def parseLine(line: String) = {
    // Split by commas into a tuple we can extract from
    val fields = line.split(",")
    // Extract the age and numFriends fields, and convert to integers
    val age = fields(2).toInt
    val numFriends = fields(3).toInt
    // Create a tuple, and return it! Wow this is weird
    (age, numFriends)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    //oh wow this is powerful, applies parseLine function to each row in the RDD
    ...
    val totalsByAge = rdd
      //take (age, numfriends), map numfriends to (age, (numfriends, 1))
      .mapValues(x => (x, 1))
    ...
    //Kicks off computation. Brings data  back from cluster so we can print to console
